...
const incrementalOutput = join(root, "deliverables", "SUPABASE_UPDATE_V13.sql");
const migrations = (await readdir(migrationsDir))
  .filter((name) => /^\d{4}_.+\.sql$/.test(name))
  .sort();

if (migrations.length !== 13) {
  throw new Error(`Expected 13 numbered migrations, found ${migrations.length}.`);
}
...
const migration13 = await readFile(join(migrationsDir, "0013_operational_usability_refund_policy.sql"), "utf8");
await writeFile(incrementalOutput, [
  "-- Eco Healthy ERP V13 — operational usability and refund-policy update",
  "-- Run this ONLY after V12 is installed (the validator must show Installed versions: 12).",
...
console.log(`Wrote ${incrementalOutput} for the current V12 staging database.`);
