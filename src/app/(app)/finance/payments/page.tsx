...
type Refund = { id: string; amount: number; reason: string; reason_code: string | null; status: string; requested_by: string; approved_by: string | null; payment_proof_reference: string | null; recipient_method: string | null; recipient_account_name: string | null; recipient_account_reference: string | null; calculation_basis: { remaining_days?: number; daily_value?: number }; invoices: { invoice_number: number } | null; customers: { full_name: string } | null };
...
export default async function Page({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  if (viewer.preview) {
    ...
    refunds = [{ id: "77777777-7777-4777-8777-777777777777", amount: 500, reason: "إلغاء جزء من الخدمة", reason_code: "OTHER", status: "requested", requested_by: "other-user", approved_by: null, payment_proof_reference: null, recipient_method: "instapay", recipient_account_name: "أحمد محمد", recipient_account_reference: "01000000000", calculation_basis: { remaining_days: 5, daily_value: 100 }, invoices: { invoice_number: 2088 }, customers: { full_name: "أحمد محمد" } }];
  } else {
    ...
    const [submissionResult, refundResult, invoiceResult, subscriptionResult] = await Promise.all([
      ...
      supabase.from("refunds").select("id,amount,reason,reason_code,status,requested_by,approved_by,payment_proof_reference,recipient_method,recipient_account_name,recipient_account_reference,calculation_basis,invoices(invoice_number),customers(full_name)").order("requested_at", { ascending: false }).limit(50),
  ...
  const canPayRefund = !viewer.preview && viewer.permissions.includes("refunds.pay");
  return <div className="space-y-7">
    ...
    <Card><CardHeader title="Refund Queue" description="طلب محسوب → اعتماد Finance مختلف عن الطالب → دفع بمرجع إثبات → Cash Reversal وCommission Clawback." /><div className="divide-y divide-[var(--border)]">{refunds.map((refund) => <div key={refund.id} className="grid gap-3 p-5 lg:grid-cols-[1fr_auto]"><div><div className="flex flex-wrap items-center gap-2"><strong>INV-{refund.invoices?.invoice_number} · {refund.customers?.full_name}</strong><Badge tone={refund.status === "paid" ? "success" : "warning"}>{refund.status}</Badge><Link href={`/finance/refunds/${refund.id}`} className="text-xs font-bold text-[var(--primary)] underline">عرض/طباعة طلب الاسترداد</Link></div><p className="mt-1 text-sm text-[var(--text-muted)]">{refund.amount} EGP · {refund.reason}</p><p className="mt-1 text-xs text-[var(--text-muted)]">{refund.calculation_basis?.remaining_days ?? "—"} يوم × {refund.calculation_basis?.daily_value ?? "—"} EGP · {refund.recipient_method} · {refund.recipient_account_name} / {refund.recipient_account_reference}</p></div><div className="flex flex-wrap gap-2">{refund.status === "requested" && canApproveRefund && refund.requested_by !== viewer.id ? <form action={approveRefundAction} className="flex items-center gap-2"><input type="hidden" name="refundId" value={refund.id} /><label className="text-xs font-bold"><input type="checkbox" required /> تأكيد</label><Button type="submit" size="sm">Approve</Button></form> : null}{refund.status === "approved" && canPayRefund && refund.requested_by !== viewer.id ? <form action={payRefundAction} className="flex flex-wrap items-center gap-2"><input type="hidden" name="refundId" value={refund.id} /><input name="proofReference" required placeholder="مرجع/رابط إثبات الدفع" className="h-9 rounded-xl border border-[var(--border)] px-3 text-sm" /><label className="text-xs font-bold"><input name="confirm" type="checkbox" value="yes" required /> تأكيد الدفع</label><Button type="submit" size="sm" variant="secondary">Pay & Clawback</Button></form> : null}</div></div>)}{!refunds.length ? <p className="p-5 text-sm text-[var(--text-muted)]">لا توجد طلبات استرداد.</p> : null}</div></Card>
  </div>;
}
