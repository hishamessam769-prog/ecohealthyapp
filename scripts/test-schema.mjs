...
sql=sql.replace(/create extension if not exists pgcrypto(?: with schema extensions)?;/g,"").replace(/create extension if not exists btree_gist;/g,"")
...
if(migrations.rows[0].count!==13)throw new Error(`Expected 13 migrations, got ${migrations.rows[0].count}`);
